---
title: "5.3.1 Access AWS Budgets"
date: 2026-07-05T21:01:00+07:00
draft: false
weight: 3
description: "Instructions on how to access the AWS Budgets management interface."
---

To begin the configuration, you can access the budget management page using one of the following two methods:

* **Method 1:** In the AWS Console search bar, type `Budgets` and select the **AWS Budgets** service.
* **Method 2:** Follow the navigation path:  
  **Billing and Cost Management** ➔ **Budgets**:

![Billing and Cost Management Home Dashboard](/images/5-Workshop/5.3-create-budget/truy-cap-aws-budgets.png)
> *Figure 5.3.1: The Billing and Cost Management home interface.*